...
export function LoginForm() {
  ...
  const handleExistingUserLogin = async (e) => {
    ...
    const isValidIdentifier =
      identifier === 'admin@gmail.com' || identifier === '1234567890' || identifier === 'APP12345';
    if (isValidIdentifier && password === 'admin') {
      toast.success(`OTP sent to ${identifier.includes('@') ? 'email' : identifier.match(/^\d{10}$/) ? 'mobile' : 'application number'}! Please verify...`, { autoClose: 2000 });
      setTimeout(() => {
        setOtp('123456');
        setIsOtpModalOpen(true);
      }, 2000);
  ...
  const handleNewUserLogin = async (e) => {
    ...
    const isValidNewUserIdentifier =
      newUserIdentifier === '9876543210' || newUserIdentifier === 'newuser@gmail.com';
    if (isValidNewUserIdentifier && password === 'newuser') {
      toast.success(`OTP sent to ${newUserIdentifier.includes('@') ? 'email' : 'mobile'}! Please verify...`, { autoClose: 2000 });
      setTimeout(() => {
        setOtp('123456');
        setIsOtpModalOpen(true);
      }, 2000);
        ...
        <div className="relative z-10">
          ...
          >
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-indigo-600">
              {loginMode === 'existing' ? 'Existing IETE Members' : 'New User Login'}
            </span>
          </motion.h2>
          ...
          <motion.div variants={itemVariants} className="mb-6 sm:mb-8">
            <div className="relative flex justify-center items-center bg-gray-100/80 rounded-full p-1 w-full max-w-[240px] sm:max-w-[300px] md:max-w-[360px] mx-auto shadow-sm">
              ...
              <button
                ...
                className={`relative z-10 w-1/2 py-2 text-xs sm:text-sm font-semibold rounded-full transition-colors duration-300 ${loginMode === 'existing' ? 'text-white' : 'text-gray-600'
                  }`}
              >
                Existing Users
              </button>
              <button
                ...
                className={`relative z-10 w-1/2 py-2 text-xs sm:text-sm font-semibold rounded-full transition-colors duration-300 ${loginMode === 'new' ? 'text-white' : 'text-gray-600'
                  }`}
              >
                New User
              </button>
          ...
          {loginMode === 'existing' ? (
            <form onSubmit={handleExistingUserLogin} className="space-y-5 sm:space-y-6">
              ...
              <motion.div variants={itemVariants}>
                ...
                <input
                  ...
                  placeholder="Enter email, mobile, member id"
                />
              </motion.div>
            ...
            <form onSubmit={handleNewUserLogin} className="space-y-5 sm:space-y-6">
              {/* Mobile Number / Email */}
              <motion.div variants={itemVariants}>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">
                  Mobile Number / Email
          ...
          >
            ...
            <a href="/register" className="text-blue-600 hover:text-blue-700 font-semibold transition-colors duration-300">
              Register now
            </a>
          </motion.p>
        </div>
